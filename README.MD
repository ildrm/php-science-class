...
- [Scientific Calculations Library](#scientific-calculations-library)
  - [Table of Contents](#table-of-contents)
  - [Installation](#installation)
  - [Classes](#classes)
    - [MathCalculations](#mathcalculations)
      - [Advanced Algebra](#advanced-algebra)
      - [Calculus Mastery](#calculus-mastery)
    ...
    - [ChemistryCalculations](#chemistrycalculations)
      ...
      - [Bonding and Molecular Shape](#bonding-and-molecular-shape)
...
#### Calculus Mastery
- **Second Derivative**  
  Numerically computes the second derivative using finite differences (f''(x) ≈ [f'(x+h) - f'(x-h)] / (2h)), enabling analysis of curvature and inflection points (`secondDerivative`). This is a step beyond basic derivatives, critical for optimization.
- **Integration by Parts**  
  Implements the advanced calculus technique ∫ u dv = uv - ∫ v du numerically, handling complex integrands with nested function evaluations (`integralByParts`), a hallmark of sophisticated integration.

#### Matrix Operations
- **Matrix Inverse**  
  Calculates the inverse of a square matrix using the adjoint and determinant (A^-1 = adj(A) / det(A)), involving recursive minor computations (`inverse`). This is a computationally intensive operation central to linear algebra solutions.
- **Determinant**  
  Recursively computes the determinant via cofactor expansion for any n × n matrix, showcasing recursive algorithmic depth (`determinant`).
...
#### Mechanics - Dynamics
- **Gravitational Force**  
  Applies Newton's universal gravitation (F = G * (m1 * m2) / r^2) with unit conversions, handling astronomical-scale calculations (`gravitationalForce`). This is foundational for orbital mechanics.
- **Torque**  
  Computes torque (τ = F * r * sin(θ)) with angular considerations and unit flexibility, critical for rotational dynamics (`torque`).

#### Thermodynamics
- **Work in Isothermal Process**  
  Calculates work (W = nRT * ln(V2/V1)) for an ideal gas under isothermal conditions, integrating logarithmic complexity with unit conversions (`workIsothermal`). This is essential for thermodynamic cycles.
- **Carnot Efficiency**  
  Determines maximum efficiency (η = 1 - T_cold / T_hot) with temperature validation, a pinnacle of thermodynamic analysis (`carnotEfficiency`).

#### Electricity and Magnetism
- **Faraday’s Law**  
  Computes induced EMF (ε = -dΦ_B/dt) for electromagnetic induction, handling dynamic flux changes (`faradayLaw`). This is key to understanding electromagnetic generators.
- **Electric Field Energy**  
  Calculates energy density in an electric field (U = (1/2) * ε_0 * E^2 * V), integrating field strength and volume (`electricFieldEnergy`), vital for capacitor design.

#### Relativity
- **Relativistic Velocity Addition**  
  Applies the relativistic formula (u' = (u + v) / (1 + (uv/c^2))) to combine velocities near light speed, ensuring physical constraints (`relativisticVelocityAddition`). This is a cornerstone of special relativity.
- **Relativistic Momentum**  
  Computes momentum (p = γ * m * v) with Lorentz factor (γ = 1 / sqrt(1 - (v^2/c^2))), reflecting high-energy physics precision (`relativisticMomentum`).

#### Quantum Mechanics
- **Quantum Probability**  
  Approximates probability via numerical integration of a wave function’s squared magnitude (P = ∫ |ψ|^2 dx), a complex quantum mechanics application (`quantumProbability`).
- **Quantum Energy Level**  
  Calculates energy in a particle-in-a-box model (E = (n^2 * h^2) / (8 * m * L^2)), integrating quantum principles (`quantumEnergyLevel`).
...
#### Thermodynamics
- **Gibbs Free Energy**  
  Computes ΔG = ΔH - T * ΔS with multi-unit support, essential for predicting reaction spontaneity (`gibbsFreeEnergy`). This integrates enthalpy, entropy, and temperature dynamically.
- **Clausius-Clapeyron**  
  Calculates vapor pressure changes (ln(P2/P1) = -ΔH_vap / R * (1/T2 - 1/T1)), a sophisticated tool for phase transitions (`clausiusClapeyron`).
...
#### Physical Chemistry
- **Ideal Gas Volume**  
  Solves for volume using the ideal gas law (V = nRT / P) with inherited precision from PhysicsCalculations, handling complex unit conversions (`idealGasVolume`).
- **pH Calculation**  
  Computes pH (-log10[H^+]) with concentration unit flexibility, critical for acid-base chemistry (`pH`).
